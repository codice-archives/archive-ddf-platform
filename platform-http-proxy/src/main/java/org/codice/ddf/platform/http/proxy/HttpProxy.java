...
public class HttpProxy {
    ...
    Properties getProperties() throws IOException {
        ...
        if (paxConfig.exists()) {
            FileReader paxReader = null;
            FileReader cxfReader = null;

            try {
                paxReader = new FileReader(paxConfig);
                cxfReader = new FileReader(cxfConfig);
                properties.load(paxReader);
                properties.load(cxfReader);
            } finally {
                IOUtils.closeQuietly(paxReader);
                IOUtils.closeQuietly(cxfReader);
            }
        }
        return properties;
    }
        ...
        public void rewrite(Exchange exchange) throws IOException {
            try {
                ...
                if (httpMessage != null) {
                    ...
                    if (body != null) {
                        String bodyStr;
                        if (httpMessage.getBody() instanceof InputStream) {
                            bodyStr = IOUtils.toString((InputStream) httpMessage.getBody());
                        } else if (httpMessage.getBody() instanceof String) {
                            bodyStr = (String) httpMessage.getBody();
                        } else {
                            bodyStr = httpMessage.getBody().toString();
                        }
